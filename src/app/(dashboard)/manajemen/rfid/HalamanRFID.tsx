...
import { PencilSquare, Trash, Search, ExclamationTriangleFill, TagFill } from "react-bootstrap-icons"
...
// [PERBAIKAN] Menggunakan skema Zod baru dari backend
...
// Fetcher function for RFID list.
...
// Komponen Modal Tambah RFID
const RfidAddModal = ({ show, onHide, onUpdate }: { show: boolean; onHide: () => void; onUpdate: () => void; }) => {
...
// Komponen Modal Edit RFID
const RfidEditModal = ({ show, onHide, rfid, onUpdate }: { show: boolean; onHide: () => void; rfid: Rfid | null; onUpdate: () => void; }) => {
    const [number, setNumber] = useState('');
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        // [PERBAIKAN] Validasi menggunakan skema update baru
        ...
        try {
            // [PERBAIKAN] Mengirim data sebagai JSON dengan method PATCH
            const payload = {
                number: number,
                is_active: String(isActive) // Kirim sebagai string "true" atau "false"
            };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.data && response.data.status === 'success') {
                ...
                // [PERBAIKAN] Logika untuk pindah halaman jika halaman saat ini menjadi kosong
                const isLastItemOnPage = data?.data.length === 1 && pagination.pageIndex > 0;
                if (isLastItemOnPage) {
                    // Pindah ke halaman sebelumnya
                    setPagination(p => ({ ...p, pageIndex: p.pageIndex - 1 }));
                } else {
                    // Tetap di halaman ini dan segarkan data
                    mutate(swrKey);
                }
